...
#+BEGIN_SRC emacs-lisp
  (defun aj/config--ycmd ()
    "Do configuration for ycmd."
    (set-variable 'ycmd-server-command '("python" "/Users/andrewjacobson/.emacs.d/misc/ycmd/ycmd"))
    (set-variable 'ycmd-global-config (expand-file-name "~/.ycm_conf.py")))

  (use-package ycmd
    :ensure t
    :hook (prog-mode . ycmd-mode)
...
Install and setup =flycheck-mode=.
...
Setup and install =flycheck-ycmd=.
